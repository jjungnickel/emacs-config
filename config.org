...
If you're looking for inspiration for your own [[https://github.com/hlissner/doom-emacs][doom-emacs]] configuration, you've
come to the wrong place. This is a butchered variant of [[https://github.com/tecosaur][tecosaur's]] most
excellent [[https://tecosaur.github.io/emacs-config/][Doom Emacs Configuration]] with most things, especially a lot of
prose and LaTeX-related things removed.

If you find anything useful, either in this document or at the original site,
consider [[https://www.buymeacoffee.com/tecosaur][buying him a coffee]].

With that out of the way, enjoy what you may find.

** Prerequisites
While most functionality of this configuration is either self-contained or
managed by doom-emacs module/package system, there are a few things that have to
be installed or configured outside of emacs.

What follows is a list of these things with me trying to explain why these are
needed. For users of [[https://archlinux.org/][Arch Linux]] and derivatives like [[https://manjaro.org/][Manjaro]], I will provide
the necessary commands to install and configure these things.
...
+ Fonts
  #+begin_src shell :eval no :tangle "setup.sh"
sudo yay -Sy otf-overpass ttf-ibm-plex-mono-git
  #+end_src
+ The =theme-magic= package requires the ~wal~ (=pywal=) executable.
  #+begin_src shell
sudo yay -Sy python-pywal
  #+end_src
...
src_shell{rm -rf~/.emacs.d~ before}. I also want to cache as much as possible, as
...
setting, and load it if it exists. I also _(ab)use_ this file for really private
configuration encrypted by ~GPG~.
#+begin_src emacs-lisp
...
**** Font Face :ATTACH:
:PROPERTIES:
:ID:       7bc6d00e-4ab4-4fbc-8acb-67ed3b352d27
:END:
...
trying out Iosevka though. So far I have mixed feelings on it, some
aspects are nice, but on others I prefer Fira.

Many distribution-packages for Iosevka don't contain all variants that I like,
so I just download the [[https://github.com/be5invis/Iosevka/releases][latest release]] and unpack the =.ttc= file into =~/.local/share/fonts/=

#+begin_src emacs-lisp
(setq doom-font (font-spec :family "Iosevka Fixed" :size 26 :weight 'light)
      doom-big-font (font-spec :family "Iosevka Fixed" :size 40)
...
(defvar required-fonts '("Iosevka Fixed" "Overpass" "IBM Plex Mono" "Merriweather" "Alegreya"))
...
**** Theme and modeline
~doom-one~ is nice and all, but I find the ~dracula~ theme just a bit nicer. Oh, and with the
...
(after! centaur-tabs
  ...
  (centaur-tabs-change-fonts "Overpass" 120)
  )
...
#+begin_src emacs-lisp
(defvar mu4e-from-name "Jan Jungnickel"
...
#+end_src
***** TODO Kubernetes
I'd really like to apply kubernetes manifests right from _source_ blocks. For
this, there is [[https://github.com/ifitzpat/ob-kubectl][GitHub - ifitzpat/ob-kubectl: Org babel extension to apply
kubectl to org bab...]] and should be applied like this:
#+begin_src emacs-lisp :tangle no
...
(defvar org-babel-lang-list
  '("go" "python" "ipython" "bash" "sh" "beancount"))
...
#+end_src
***** Cliplink
When inserting links from the clipboard into an org-buffer via =org-cliplink= we
can customize the way the link title is generated. By default, the title is
taken from the site. This can be improved for some cases, for instance a link to
a GitLab merge-request can be expressed as [[https://gitlab.com/some/project/-/merge_requests/123][!123]] or [[https://gitlab.com/some/project/-/merge_requests/123][some/project!123]]
...
(setq org-agenda-files
      (nconc
       (list "~/org/todo.org" "~/org/projects.org" "~/org/inbox.org")
       (org-get-agenda-files-recursively "~/org/work/journal/")))
...
#+end_src
**** Capture
On mobile, I'm using [[https://beorg.app/][beorg - Task management for iPhone and iPad]] to view and
capture tasks. When quickly capturing tasks, the most direct flow only lets me
configure a file, but not the heading where to add the new item.

I want to have the same target file on all platforms and =todo.org= is cluttered
enough, so let's drop all newly added items into =inbox.org=:
...
#+begin_src emacs-lisp :noweb-ref none :tangle yes
(setq org-roam-directory "~/Documents/Organisation/Roam/")
#+end_src
...
#+begin_src emacs-lisp :noweb-ref none :tangle (if (file-exists-p "~/Documents/Organisation/Roam/") "no" "packages.el")
...
:config
 (setq org-pretty-tags-surrogate-strings
       `(("chore"      . ,(all-the-icons-material "cached"         :face 'all-the-icons-purple  :v-adjust 0.01))
         ("finances"   . ,(all-the-icons-material "monetization_on" :face 'all-the-icons-silver :v-adjust 0.01))
         ...
         ("people__michael_pilz" . ,(all-the-icons-fileicon "pickle" :face 'all-the-icons-red :v-adjust 0.01))))
...
#+end_src
** Beancount
#+begin_src shell :tangle ~/.local/bin/beancount-language-server :tangle-mode (identity #o755)
#!/usr/bin/env bash
beancount-language-server
#+end_src
#+begin_src emacs-lisp
(setq
 lsp-beancount-langserver-executable "~/.local/bin/beancount-language-server")
#+end_src
